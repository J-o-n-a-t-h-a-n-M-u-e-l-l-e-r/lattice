...
Lattice infers dependency edges between GitHub issues, gets a human to approve them, writes them into GitHub's native `blocked_by` model, and serves the resulting schedule to coding agents over MCP.
...
1. **Never write a GitHub dependency from agent code.** Use `propose_dependency`, which queues for human review. This is the project's central claim.
2. **Never weaken the DAG invariant** in `src/graph/acyclic.ts` to make something pass.
3. **Never drop the `evidence` field** or stop validating it against source text — it's what makes the human gate real rather than decorative.
...
- `propose_dependency(...)` if you discover a blocker that isn't recorded. **Do not work around it silently.**
