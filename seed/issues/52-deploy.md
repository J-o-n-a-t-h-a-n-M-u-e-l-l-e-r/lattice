...
- **Backend** — pipeline, store, REST API, MCP. Needs `DATABASE_URL` (Neon), `OPENROUTER_API_KEY`, the GitHub token, `LATTICE_API_TOKEN`, `LATTICE_MCP_TOKEN`.
...
Then point the GitHub webhook at the backend's `/api/webhook`. Publish the MCP server URL for external clients that need the scheduling tools.
...
2. **External MCP clients need a public HTTP endpoint.** No deploy, no remote MCP integration.
...
- `vercel.json` if needed, env configuration, webhook setup, and deploy notes in the README
...
- [ ] An external MCP client can reach `/mcp` and list tools
