...
`GET /api/graph`, `GET /api/runs`, `GET /api/runs/:id`, `GET /api/issues/:number`, `GET /api/edges/:blocked/:blockedBy`, plus `POST /api/runs`, `/api/webhook` and `/api/dispatch`. Bearer auth via `LATTICE_API_TOKEN`.
